# 商品系置き換え方針（旧 `backend/api` → `backend`）

本ドキュメントは、商品関連APIの置き換え作業を安全に進めるための実装方針をまとめたものです。  
`backend/docs/ddd-and-clean-architecture.md` の方針を前提とし、商品ドメイン特有の複雑性（画像・CSV・複数テーブル参照）に対する具体案を定義します。

## 1. 前提と目的

- 旧 API は `backend/api` 構成で稼働していた。
- 本作業では、以下の商品関連ルートを `backend` へ移行する。
...
- エラー形式は既存 `backend` の統一レスポンスに合わせる:
  - `{ "message": "..." }`
- 管理系の更新APIは既存どおり認証必須。

## 10. 受け入れ条件（Definition of Done）

- 置き換え対象ルートが `backend` 側で仕様どおり動作する
