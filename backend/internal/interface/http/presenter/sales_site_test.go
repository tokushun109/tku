...
func TestToSalesSiteResponse(t *testing.T) {
	t.Run("SalesSiteを渡したならSalesSiteResponseを返す", func(t *testing.T) {
		...
		salesSite, err := domain.Rebuild(1, u.Value(), n.Value(), salesSiteURL.Value())
		if err != nil {
			t.Fatalf("unexpected error: %v", err)
		}
		res := ToSalesSiteResponse(salesSite)
		if res.UUID != u.Value() || res.Name != n.Value() || res.URL != salesSiteURL.Value() {
...
func TestToSalesSiteResponses(t *testing.T) {
	t.Run("SalesSite配列を渡したならSalesSiteResponse配列を返す", func(t *testing.T) {
		...
		salesSite1, err := domain.Rebuild(1, u1.Value(), n1.Value(), url1.Value())
		if err != nil {
			t.Fatalf("unexpected error: %v", err)
		}
		salesSite2, err := domain.Rebuild(2, u2.Value(), n2.Value(), url2.Value())
		if err != nil {
			t.Fatalf("unexpected error: %v", err)
		}
