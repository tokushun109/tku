...
func TestToSalesSiteResponse(t *testing.T) {
	t.Run("SalesSiteを渡したならSalesSiteResponseを返す", func(t *testing.T) {
		...
		salesSite, err := domain.Rebuild(1, u.String(), n.String(), salesSiteURL.String(), "icon")
		if err != nil {
			t.Fatalf("unexpected error: %v", err)
		}
		res := ToSalesSiteResponse(salesSite)
		if res.UUID != u.String() || res.Name != n.String() || res.URL != salesSiteURL.String() || res.Icon != "icon" {
...
func TestToSalesSiteResponses(t *testing.T) {
	t.Run("SalesSite配列を渡したならSalesSiteResponse配列を返す", func(t *testing.T) {
		...
		salesSite1, err := domain.Rebuild(1, u1.String(), n1.String(), url1.String(), "icon1")
		if err != nil {
			t.Fatalf("unexpected error: %v", err)
		}
		salesSite2, err := domain.Rebuild(2, u2.String(), n2.String(), url2.String(), "icon2")
		if err != nil {
			t.Fatalf("unexpected error: %v", err)
		}
